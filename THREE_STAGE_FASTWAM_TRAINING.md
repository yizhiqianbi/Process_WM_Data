...
本文是当前七个机器人数据集接入 FastWAM 的执行手册。范围固定为 FastWAM，不包含
...
1. **Stage 1: video backbone pretrain**。使用七个数据集的 A/B 级视频、语言和相机布局，
...
- 七个数据集都进入 Stage 1，并已逐库完成真实解码验收。
- 七个数据集都能进入 MemoryFastWAM 数据路径。
- 七库 Stage 1/2 已使用 dataset-balanced sampler 重新完成真实 5B/6B 训练回归。
- MemoryFastWAM action inference 已使用真实 8/2/1 历史和 Stage 3 checkpoint 验收。
- 当前预处理实测中，OXE ASU、OXE-AugE、RoboCOIN、RoboMIND、Galaxea 和 InternData-A1
  已能产出动作监督 A 级 case；AgiBot 因本地缺 `proprio_stats` 仍是视频监督 B 级。
- B 级 case 的 action/state 张量为零且所有维度 mask 为无效，`action_loss_mask=false`。
- 任何数据集只有在本体映射、时间对齐和 action 语义确认后，才允许升级到 A 级。
- 上述六域是数据预处理验收结果；现有 Stage 2 normalization stats 仍只覆盖 RoboCOIN，六域
  联合 optimizer/checkpoint 回归尚未重跑，不能把两种状态混为一谈。
...
| AgiBot-Beta | observation tar 内 AV1 | 20 | `1,1,1` | B | 通过 | 通过 | 禁用 |
...
逐库验收选择第一个 `window_start > 0` 的窗口。当前七库均为 `window_start=40`，因此
memory 取 canonical index 29 至 39，11 个 index 全部小于 40。所有样本均得到有限值的
`[3,21,384,320]` 当前视频和 8/2/1 memory 视频。
...
当前全库配置是 `configs/data/canonical_stage2_memory_all.yaml`。其中 normalization stats
仍只覆盖历史 A 级 RoboCOIN domain。预处理新开放的五个 action domain 不能直接接到正式
Stage 2；必须先在全量 train split 上分别重算 stats、更新 domain 路由，再执行联合
optimizer/checkpoint/resume 回归。B 级样本不查询 action/state stats。
...
七库 memory smoke 使用 `sample_offset_per_case=1` 和 `max_samples_per_case=1`，严格得到
七个 `window_start=40` 的不同数据源样本。7/7 step 均为 `memory_valid_ratio=1.0`；六个 B 级
step 的 `loss_action=0`，唯一 RoboCOIN A 级 step 的 `loss_action=2.9899`。最终 checkpoint：
...
- 当前 observation tar 的三路视频可用，observation/proprio join 代码和合成测试已完成。
- 等下载补齐 `proprio_stats`、`parameters` 和 `task_info` 后执行真实 join。
- 校验 observation/action episode ID、帧数和纳秒时间戳一一对应。
...
memory-aware action inference。2026-07-17 新增的动作准入又证明六个本地控制域能生成统一
81/80/21 case，但尚未使用新六域 stats 重跑 Stage 2。当前 manifest 仍是逐库真实样本级验收集，
