...
CPU ray tracing，CuRobo 和 FastWAM 仍使用 CUDA。完整 9 x 100 run 显式设置 `1 spp`、
`depth 1` 和 `denoiser none`；官方环境使用 GPU renderer 和 OIDN，两者不是
pixel-equivalent。renderer 参数
写入每个 receipt，并由比较脚本动态读取，不允许在报告中硬编码成默认 32/8。

Mesa/SAPIEN 的 `take_picture/get_picture` 在同一进程内稳定，但多个 lavapipe simulator
并发读取 Color buffer 会随机触发 native segmentation fault。评测因此保留 8 卡模型并行，
同时使用一把主机级 `fcntl` 文件锁串行化每个 observation 的完整
`update_render -> take_picture -> get_picture` 事务。模型 forward、CuRobo、物理步进和结果
写入不持有该锁。每个任务仍使用独立 XDG cache/tmp/Mesa shader cache；torch、BLAS 和
...
```bash
RMBENCH_RT_SAMPLES=1 \
RMBENCH_RT_PATH_DEPTH=1 \
RMBENCH_RT_DENOISER=none \
RMBENCH_RENDER_LOCK_PATH=/tmp/rmbench-lavapipe-render-$UID.lock \
...
rollout。progress identity 同时固定 checkpoint、renderer 和 observation stride；任一结果协议
变化都会拒绝读取旧进度。渲染锁只改变任务间调度顺序，不改变单个 observation 的相机、分辨率
或像素后处理。
