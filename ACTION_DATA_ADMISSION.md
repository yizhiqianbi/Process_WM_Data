...
本地已有控制数据的六个数据集都已生成 `joint_video_action` 样本：
...
| AgiBot-Beta | 暂不开放 | 本地只有 observation tar，缺 `proprio_stats` | 仅 video |
...
本地下载内容目前只有 `observations/<task>/*.tar`。其中三路 AV1 视频和 depth frame 可用，但对应的 `proprio_stats` 控制 tar 尚未出现。缺少控制文件时，无法构造 state/action，更不能只凭 observation 帧数开放 action loss。

当前明确 blocker：

```text
parameters_directory_missing
proprio_stats_archives_missing_or_incomplete
task_info_directory_missing
```

**控制分片到达后的自动处理**

adapter 已实现按 episode ID 将 observation tar 与 `proprio_stats.h5` join，并按官方 HDF5 contract 读取：
...
| 同上 | 12 | left/right hand `28:34,40:46` |
| `state/head/position` / `action/head/position` | 2 | `60,61` |
| `state/waist/position` / `action/waist/position` | 2 | `58,59` |

时间戳按纳秒解析。官方将 state 描述为传感器/执行器 feedback，将 action 描述为发送给 HAL 的 command。effector feedback 与 command 可能分别使用毫米和归一化闭合量，所以这两个 slot 标记为 `alignment_safe=false`：它们可训练，但不会用错误的同量纲假设阻断整个 episode。

合成官方布局测试已覆盖 tar join、HDF5 时间戳和 14D 双臂 action；真实 action 回归要等下载进程拿到 `proprio_stats` 后重跑。
...
自动测试当前为 29 项，覆盖 OXE 派生变换、HDF5 纳秒时间戳、AgiBot tar join、InternData 命名、局部 bad interval 和完整 pipeline contract。
...
1. 等 AgiBot `proprio_stats`、`task_info` 和 `parameters` 下载完成后，执行真实 join 回归。
...
4. 对六个已开放 domain 运行全量 manifest，统计 hard interval、alignment lag 和 action slot 覆盖率。
5. 用全量 train split 重建每个 embodiment 的 normalization statistics。
6. 更新 FastWAM Stage 2 多数据集配置和 stats 后，再做一次六域真实 optimizer/checkpoint/resume 回归。

最后一项尚未执行。当前已经证明的是数据处理链能产出六个真实 action domain 的统一 case，不应把它表述为六域 FastWAM 正式预训练已经完成。
